...
// Get all active quests (Daily, Weekly, Challenges)
...
router.get('/user/progress', authenticateJWT, async (req, res) => {
  try {
    const userQuests = await req.app.get('db').models.UserQuest.findAll({
      where: { userId: req.user.id },
      include: [
        {
          model: req.app.get('db').models.Quest,
          as: 'quest',
        },
      ],
    });
...
// Start a quest
router.post('/:questId/start', authenticateJWT, async (req, res) => {
  try {
...
// Complete a quest
router.post('/:questId/complete', authenticateJWT, async (req, res) => {
  try {
